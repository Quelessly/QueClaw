...
interface SettlementRow {
  ...
  pendingAfterSettlement: number
  pendingAmountAfterSettlement: number
}
...
export default function SettlementsPage() {
  ...
  const handleMarkSettled = async (item: SettlementRow) => {
    // If already settled but has pending orders after, settle only the new batch
    const amountToSettle = item.settled
      ? item.pendingAmountAfterSettlement
      : item.grossAmount
    const ordersToSettle = item.settled
      ? item.pendingAfterSettlement
      : item.totalOrders
    ...
    try {
      const res = await fetch(`${API}/admin/settlements/mark-settled`, {
        ...
        body: JSON.stringify({
          ...
          totalOrders: ordersToSettle,
        }),
      }).then(r => r.json())
      if (res.success) {
        // Re-fetch so settled_at resets and delta recalculates correctly
        await fetchSettlements(adminSecret, date)
      } else {
  ...
  // Unsettled vendors + post-settlement pending amounts
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4">
            ...
            <p className="text-xs text-zinc-600 mt-1">
              {data.filter(s => !s.settled && s.grossAmount > 0).length +
                data.filter(s => s.settled && s.pendingAmountAfterSettlement > 0).length} vendors
            </p>
          </div>
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4">
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    ...
                    {hasPendingAfter && (
                      <span className="text-xs font-bold bg-amber-500/10 text-amber-400 border border-amber-500/30 px-2.5 py-1 rounded-full">
                        Partially settled
                      </span>
                    )}
                  </div>
                  ...
                  {/* Post-settlement pending warning */}
                  {hasPendingAfter && (
                    <div className="mb-3 bg-amber-500/10 border border-amber-500/20 rounded-xl px-3 py-2.5">
                      <p className="text-amber-400 text-xs font-bold">
                        {item.pendingAfterSettlement} new order{item.pendingAfterSettlement > 1 ? 's' : ''} after settlement
                      </p>
                      <p className="text-amber-300/70 text-xs mt-0.5 font-mono">
                        Rs.{item.pendingAmountAfterSettlement} not yet paid
                      </p>
                    </div>
                  )}

                  {/* Action buttons — show for unsettled OR for settled-with-pending */}
                  {(!item.settled && item.grossAmount > 0) || hasPendingAfter ? (
                    <div className="flex gap-2">
                      {item.vendor.upi_id ? (
                        <a
                          href={`upi://pay?pa=${item.vendor.upi_id}&pn=${encodeURIComponent(item.vendor.name)}&am=${
                            hasPendingAfter ? item.pendingAmountAfterSettlement : item.grossAmount
                          }&cu=INR`}
                      ...
                      >
                        {settling === item.vendor.id
                          ...
                          : hasPendingAfter
                          ? `Settle Rs.${item.pendingAmountAfterSettlement}`
                          : 'Mark Settled'}
                      </button>
                    </div>
                  ) : null}
