  ...
  return (
    <div style={{ minHeight: '100vh', background: '#FAF7F2', paddingBottom: 120, fontFamily: 'var(--font-dm-sans), sans-serif' }}>
      ...
      <div style={{ padding: '56px 20px 24px' }}>
        <button onClick={() => router.back()}
          style={{ fontSize: 14, color: '#8a7f72', fontWeight: 500, marginBottom: 16, display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'var(--font-dm-sans), sans-serif' }}>
          ← back
        </button>
        <h1 style={{ fontFamily: 'var(--font-fraunces), serif', fontStyle: 'italic', fontWeight: 700, fontSize: 32, color: '#1a1714', letterSpacing: '-1px', margin: '0 0 4px' }}>your order.</h1>
        <p style={{ color: '#8a7f72', fontSize: 14, margin: 0, fontFamily: 'var(--font-dm-mono), monospace' }}>{itemCount} item{itemCount !== 1 ? 's' : ''}</p>
      </div>
      ...
      <div style={{ padding: '0 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        <div style={{ background: '#fff', borderRadius: 20, padding: 24, border: '1px solid rgba(26,23,20,0.07)', boxShadow: '0 2px 12px rgba(26,23,20,0.04)' }}>
          <p style={{ fontSize: 10, fontWeight: 600, color: '#8a7f72', letterSpacing: 2, textTransform: 'uppercase', marginBottom: 16, fontFamily: 'var(--font-dm-mono), monospace' }}>Order Summary</p>
          <div>
            {cart.map((item, i) => (
              <div key={item.id}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 0' }}>
                  <div>
                    <p style={{ color: '#1a1714', fontWeight: 500, fontSize: 14, margin: '0 0 2px', fontFamily: 'var(--font-dm-sans), sans-serif' }}>{item.name}</p>
                    <p style={{ color: '#8a7f72', fontSize: 12, margin: 0, fontFamily: 'var(--font-dm-mono), monospace' }}>₹{item.price} × {item.quantity}</p>
                  </div>
                  <p style={{ fontWeight: 700, color: '#1a1714', fontFamily: 'var(--font-dm-mono), monospace', fontSize: 15, margin: 0 }}>₹{item.price * item.quantity}</p>
                </div>
          ...
          <div style={{ borderTop: '1px solid rgba(26,23,20,0.08)', marginTop: 4, paddingTop: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ color: '#8a7f72', fontSize: 14, fontFamily: 'var(--font-dm-sans), sans-serif' }}>Item total</span>
              <span style={{ color: '#1a1714', fontFamily: 'var(--font-dm-mono), monospace', fontSize: 14 }}>₹{total}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ color: '#8a7f72', fontSize: 14, fontFamily: 'var(--font-dm-sans), sans-serif' }}>Convenience fee</span>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ color: '#c9c2b8', fontSize: 12, textDecoration: 'line-through', fontFamily: 'var(--font-dm-mono), monospace' }}>₹5</span>
                <span style={{ color: '#ff6b00', fontWeight: 700, fontSize: 14, fontFamily: 'var(--font-dm-sans), sans-serif' }}>FREE</span>
              </div>
            </div>
            <div style={{ borderTop: '1px solid rgba(26,23,20,0.08)', paddingTop: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ color: '#1a1714', fontWeight: 700, fontSize: 16, fontFamily: 'var(--font-dm-sans), sans-serif' }}>Total</span>
              <span style={{ color: '#1a1714', fontWeight: 800, fontSize: 22, fontFamily: 'var(--font-dm-mono), monospace' }}>₹{total}</span>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, background: 'rgba(255,107,0,0.05)', border: '1px solid rgba(255,107,0,0.15)', borderRadius: 14, padding: '14px 16px' }}>
          ...
          <p style={{ fontSize: 13, color: '#8a7f72', lineHeight: 1.7, margin: 0, fontFamily: 'var(--font-dm-sans), sans-serif' }}>
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, padding: 20, background: 'linear-gradient(to top, #FAF7F2 60%, transparent)' }}>
        <button onClick={placeOrder} disabled={loading}
          style={{ width: '100%', background: '#ff6b00', color: '#fff', padding: '16px 0', borderRadius: 16, fontWeight: 700, fontSize: 16, border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, boxShadow: '0 8px 24px rgba(255,107,0,0.3)', opacity: loading ? 0.7 : 1, transition: 'all 0.2s', fontFamily: 'var(--font-dm-sans), sans-serif' }}>
          {loading ? (
            <>
