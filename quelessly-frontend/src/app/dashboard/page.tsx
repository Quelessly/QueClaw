...
function DashboardShell({
  ...
  useEffect(() => {
    ...
    return () => {
      if (socket) { socket.off('new_order'); socket.off('order_updated') }
    }
  }, [token, vendor?.id])
        ...
        {TABS.map((t) => (
          <button key={t.key} onClick={() => setTab(t.key)} title={t.label}
            className={`relative w-12 h-12 rounded-2xl flex items-center justify-center text-xl transition-all duration-200 ${
              tab === t.key ? 'bg-lime-400/10 text-lime-400 shadow-[0_0_12px_rgba(163,230,53,0.15)]' : 'text-zinc-600 hover:text-zinc-300 hover:bg-zinc-900'
            }`}>
            {t.icon}
            {t.badge !== undefined && (
...
function OrdersTab({
  ...
  if (loading) return (
    <div className="space-y-3">{Array.from({ length: 3 }).map((_, i) => <SkeletonOrderCard key={i} />)}</div>
  )
...
function OrderCard({
  ...
  return (
    <div className={`bg-zinc-900 rounded-3xl overflow-hidden border transition-all duration-300 ${
      isNew ? 'border-lime-400/50 shadow-[0_0_20px_rgba(163,230,53,0.1)]' : sla ? 'sla-alert border-rose-500' : 'border-zinc-800'
    }`}>
...
function InlineField({ value, type = 'text', prefix = '', onSave }: {
  value: string; type?: string; prefix?: string; onSave: (v: string) => void
}) {
...
function MenuItemRow({ item, onToggle, onDelete, onEdit, onInlineEdit }: {
  item: MenuItem
